    ...
    productAddToCartButtons.map(productAddToCartButton => {
        productAddToCartButton.onclick = function (e){
            ...
            let currenElement = e.currentTarget,
                ...
                // users[indexOfUser].userProducts  = []
                users[indexOfUser].userProducts ? checkUsersExisitProducts ( productIndex ) : addNewProducToUserProducts()
...
function checkUsersExisitProducts ( index ){
    ...
    let exisiteProduct =    userProducts.find ((userProduct)  =>  userProduct.id == product.id )
console.log (userProducts)
console.log (product)
console.log (exisiteProduct)
        if (exisiteProduct){
    // userProducts.forEach (( userProduct , index) => {
    //         if (userProduct == exisiteProduct){
...
// })
            ...
            users[indexOfUser].userProducts = [...users[indexOfUser].userProducts , exisiteProduct ]
}
else{
    products[productIndex].numberOfProds = 1
    users[indexOfUser].userProducts = [...users[indexOfUser].userProducts , products[index] ]
}
}
function addNewProducToUserProducts(index){
    users[indexOfUser].userProducts[index].numberOfProds = 1
