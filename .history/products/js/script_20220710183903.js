...
userProducts , userOwnProducts   , productArray , numOfProducts = 0 ;
...
console.log (all.signInBtn)
function signProducts (){
    if (all.signInBtn){
        console.log ("sign in")
        all.signInBtn.onclick = function (){
            console.log ("sign in")
...
// running dorwProducts function 
 function runDrowProductsFuction(){
    if (logInUserName && indexOfUser){
        ...
        // drowProducts(productsData)
        localStorage.setItem("products" , JSON.stringify(productsData))
    }
    productArray = JSON.parse(localStorage.getItem("products"))
 }
  runDrowProductsFuction()
 drowProducts(productArray)
...
let leftCharvon = document.querySelector(".left-charvon"),
rightCharvon = document.querySelector(".right-charvon") ,
reviewItem = document.querySelector(".review-item") ,
reviewIndex = 0

function showSlider (element){
    reviewItem.innerHTML =""
    reviewItem.innerHTML = `<div class="review-item-img" >
...
rightCharvon.onclick = function (){
    if (reviewIndex >= 0 && reviewIndex < reviewData.length-1 ){
        reviewIndex ++
        showSlider (reviewData[reviewIndex])
        leftCharvon.classList.contains("active") ? "" : leftCharvon.classList.add("active")
    }
    if (reviewIndex > 0  ){
   ...
   if (reviewIndex == reviewData.length-1 ) {
        rightCharvon.classList.remove("active")
    }
}
leftCharvon.onclick = function (){
    if (reviewIndex  > 0 && reviewIndex <= reviewData.length-1 ){
        reviewIndex -=1
        showSlider (reviewData[reviewIndex])
    ...
    if ( reviewIndex < reviewData.length-1 ){
        ...
        rightCharvon.classList.contains("active") ? "" : rightCharvon.classList.add("active")
    }
   if (reviewIndex == 0 ) {
        leftCharvon.classList.remove("active")
    }
}
/**********************************************************/   
var  charvonArrow ;
