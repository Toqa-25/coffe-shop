...
(drowElements = function () {
  productGroup = JSON.parse(localStorage.getItem("products"));
  if (productGroup) {
    let drow = productGroup.map((e) => {
      return `<div id="${
        e.id
      }" class="cards-content-item" style=" margin-bottom:20px">
    <div class="cards-content-item-img" style="background-image: url(&quot;../../product/images/product-${
      e.id
    }.jpg&quot;); background-position: center center; background-size: cover; background-repeat: no-repeat;">
    <div class="cards-content-item-img-icon">
     <i class="fas fa-eye"></i>
    <hr>
    <i class="fas fa-shopping-cart add-card" onclick='addToCart(${e.id})'></i>
    <hr>
    <i class=" ${
      localStorage.getItem("favoriteProduct") && e.like ? "fas" : "far"
    }  fa-heart heart-item  " onclick='saveFavorite(${e.id}) ' ></i>
</div>
</div>
<div class="cards-content-item-info" >
<p class=" m">${e.desc}</p>
<h4 class=" m">${e.title}</h4>
<div class=" m">
        <i class="fas fa-star y-color"></i>
        <i class="fas fa-star y-color"></i>
        <i class="fas fa-star y-color"></i>
        <i class="far fa-star"></i>
        <i class="far fa-star"></i>
              </div>
              <h4 class="cards-content-item-info-price m">${e.price}</h4>
                <button class="btn" onclick='saveDetails(${
                  e.id
                })'>Details</button>
                <button class="btn" onclick='delateProduct(${
                  e.id
                })'>Delate</button><br><br>
                <button class="btn" onclick='editProduct(${e.id})'>Edit</button>

  </div>
  </div>`;
    });
    cardsContent.innerHTML = drow.join(" ");
  }
  // else {
  //   cardsContent.innerHTML="no products"
  // }
