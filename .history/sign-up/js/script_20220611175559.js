...
signUpFormUserName.onchange = checkExisitData(signUpFormUserName , neadedValue , localStorgeItemName , itemNameMsg )
...
function checkExisitData(input , neadedValue , localStorgeItemName , itemNameMsg ){
    if (input.value != ""){
       newUsers.map(newUserItem => {
           if (newUserItem.neadedValue != input.value.trim()  ){
               localStorage.setItem(localStorgeItemName , input.value.trim() )
               newUserItem.neadedValue = input.value.trim()
           }
           else{         
